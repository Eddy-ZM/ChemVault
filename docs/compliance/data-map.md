...
| Payment data if present | subscription ID, plan, usage records, checkout/portal placeholder | Main site billing placeholders | Billing and entitlements if enabled | Main D1 schema placeholders | Stripe or other processor if enabled | Not currently defined | Not currently detected | Medium | Live payment data not currently detected. Needs legal review before launch. |
