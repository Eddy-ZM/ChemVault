...
Staging may keep clearly labeled mock billing for UI QA. Production must never return fake checkout success. The source contains Stripe checkout and portal integrations, but production requests must fail closed unless verified identity, Stripe secrets, plan flags, and `ENABLE_PUBLIC_CHECKOUT=true` are all present.
...
- `/api/billing/checkout` returns `placeholder_checkout` only when staging mock billing is explicitly enabled; otherwise it uses configured Stripe or fails closed.
- `/api/billing/portal` returns `placeholder_portal` only in staging mock mode; otherwise it uses configured Stripe or fails closed.
- `/api/export/compound` returns HTTP 501 because export is not included in a purchasable plan.
...
- Production billing portal uses configured Stripe for the canonical customer or fails closed when billing is disabled or incomplete.
