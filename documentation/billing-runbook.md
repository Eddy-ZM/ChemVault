...
Enable the daily `billing-reconciliation.yml` workflow only after its dedicated secret and production URL are configured. Its first canary should change a test subscription directly in Stripe, withhold the webhook in a non-production endpoint, run reconciliation, and verify D1 and entitlements converge. Any nonzero workflow result requires operator review before checkout remains enabled.
