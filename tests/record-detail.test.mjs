...
test("record detail page uses a fresh cache key for its changed script", () => {
  ...
  assert.match(html, /record-page\.js\?v=20260611b/, "record page references the upgraded detail script cache key");
});

test("record detail page busts the stylesheet cache for new detail styles", () => {
  const html = read("pages/record.html");

  assert.match(html, /portal\.css\?v=20260611c/, "record page references the upgraded portal stylesheet cache key");
});
